...
autoplot(snaive(enplanements,lamda="auto"))
```
...
#Mit zunehmender Zeit stärker /schwächer

#Nicht bei schwankender Varianz (Bauchform)
```
Your answer:
Für Varianzen, die eine Schwankung besitzen ist ein Box.Cox transformation nicht möglich.
Die Varianz sollte städig steigen oder fallen über die Zeit.
...
Your answer:
Eine logaritmische Datentransformation bietet sich hierbei an. Die Extremwerte des Graphen werden hierbei deutlich geglättet.
...
aus_fit <- ausbeer %>% 
  ...
  #model(SNAIVE(ausbeer ~ lag("year")))

beer_fc <- aus_fit %>%
  forecast(h = "$ years")
autoplot(beer_fc, level = NULL)
...
Your answer: Falls das Vorhersagemodell normalverteilt ist können gute Vorhersagen gemacht werden. -> Wahr

> b. A model with small residuals will give good forecasts.

Your answer: Wahr -> kleinere Störgröße 

> c. The best measure of forecast accuracy is MAPE.

Your answer: Falsch. MASE 

> d. If your model doesn't forecast well, you should make it more complicated.

Your answer: Falsch. White Noise !!!
...
Your answer:
Die RMSE sollte möglichst klein ausfallen. Hierbei haben wir eine Abweichung von 12.275 %.

How sensitive are the accuracy measures to the training/test split?
Der Unterschied der Traningsdaten zu dem TestSet liegen meist über das Doppelte des Wertes (siehe Tabelle)
...
Da es hier keine Sesonale Komponente enthaltet ist: Seasonal_naive kein guter Ansatz. Naive ist die eingfachste Annahme und kann zur Fortführung des Graphen genutzt werden. Mean wird hier zwischen den zwei Extremwerten liegen ca. bei 116, was nicht umbedigt gut für eine Prognose ist.
...
autoplot(dowjones) +
  autolayer(meanf(dowjones, h=15),series="Mean 15", PI=FALSE) +
  autolayer(meanf(dowjones, h=10),series="Mean 10", PI=FALSE) +
  autolayer(meanf(dowjones, h=5),series="Mean 5", PI=FALSE) +
  xlab("Time") + ylab("Closing Price (US$)") +
  ggtitle("Dow Jones index") +
  guides(colour=guide_legend(title="Forecast"))
```
...
autoplot(dowjones) +
  autolayer(naive(dowjones, h=15),series="Naïve 15", PI=FALSE) +
  autolayer(naive(dowjones, h=10),series="Naïve 10", PI=FALSE) +
  autolayer(naive(dowjones, h=5),series="Naïve 5", PI=FALSE) +
  xlab("Time") + ylab("Closing Price (US$)") +
  ggtitle("Dow Jones index") +
  guides(colour=guide_legend(title="Forecast"))
```
...
help(ibmclose)
autoplot(ibmclose)
autoplot(BoxCox(ibmclose, lambda = "auto"))
tsdisplay(ibmclose)
```
...
```{r}
# todo
fc1<-snaive(hsales.test)
fc2<-rwf(hsales.test, drift=TRUE)
fc3<-mean(hsales.test)
fc4<-naive(hsales.test)
...
# Nach den Zahlen scheint der Drift Forecast am genausten zu sein
```
...
autoplot(y) + ggtitle("White noise")
```
Es ist nicht white noise. Extremwerte schwanken stark.
